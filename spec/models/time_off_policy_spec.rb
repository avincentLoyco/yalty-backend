...
RSpec.describe TimeOffPolicy, type: :model do
  ...
  it { is_expected.to have_db_column(:end_time).of_type(:string).with_options(null: false) }
  it { is_expected.to have_db_column(:start_time).of_type(:string).with_options(null: false) }

class RecreateBalancesHelper
  ...
  def manage_additions!
    ...
    return ManageEmployeeBalanceAdditions.new(etop_for_manage).call unless old_effective_at.present?
  ...
  def first_etop_before(date = new_effective_at)
    before_etop = etops_in_category.where('effective_at < ?', date).last
    return unless before_etop.present?
    before_etop
  end
end

class HandleContractEnd
  JOIN_TABLES = %w(employee_time_off_policies employee_presence_policies employee_working_places)
  ...
  def remove_balances
    @employee.employee_balances.where('effective_at > ?', @contract_end_date).not_time_off.delete_all
  end
  ...
  def move_time_offs
    ...
    """).update_all(end_time: @contract_end_date + 1.day)
  end

  def find_next_hire_date(employee)
    employee.events.where(event_type: 'hired').where('effective_at > ?', @contract_end_date)
